...
def _index_team_stats_by_key(stats: List[Dict[str, Any]]) -> Dict[str, Dict[str, float]]:
    """
    Helper to index team stats by team name.
    """
    index: Dict[str, Dict[str, float]] = {}
    for stat in stats:
        team_name = str(stat.get("TeamName"))
        if team_name:
            index[team_name] = {
                "win_pct": float(stat.get("WinPct", 0.0)),
                "run_diff_per_game": float(stat.get("RunDiffPerGame", 0.0)),
            }
    return index
